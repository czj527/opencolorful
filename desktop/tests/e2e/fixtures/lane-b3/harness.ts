/**
 * B3 lane · Playwright fixture 组合（与 lane-a4b harness 同语义，绑定 lane-b3 后端）。
 *
...
import { LaneB3BackendHarness } from "./backend.js";

export interface LaneB3Fixtures {
  /** 已就绪的隔离 lane 后端（serverUrl/appUrl/stubUrl/homeDir/userDataDir 可用） */
  lane: LaneB3BackendHarness;
}

export const test = base.extend<LaneB3Fixtures>({
  lane: async ({ }, use, testInfo) => {
    const lane = new LaneB3BackendHarness(sanitizeLabel(testInfo.title));
    try {
      await lane.start();
      await use(lane);
    } finally {
