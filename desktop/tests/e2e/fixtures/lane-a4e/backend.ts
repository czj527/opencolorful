/**
 * A4e lane · 后端 fixture（隔离生命周期，lane 本地实现）。
 *
 * 结构与共享冒烟 fixtures/backend.ts / lane-a4b backend.ts 同构（runRoot/home/user-data/logs +
 * ready 行解析 + dispose retain/清理自检），差异：
 * - 引导的是 lane-a4e/server-bootstrap.ts（脚本化 spawn_subagent stub，无 circuit proxy）；
 * - app 直连 Agent Server（appUrl === serverUrl）；真值断言同样直连（只读）；
 * - stub 控制面：configureScript()（spawn 参数 + 状态复位）、releaseChild()（放行
 *   report_subagent_result）、stubState()（请求分类日志，诊断/报告证据）。
 *
 * 复用共享 backend.ts 的导出（只读 import）：ARTIFACTS_DIR / REPO_ROOT / stripCredentialEnv。
 */
...
export interface StubScriptConfigInput {
  readonly providerId?: string;
  readonly modelId?: string;
}

export interface StubRequestClassification {
  readonly n: number;
...
export interface StubState {
  readonly providerId: string;
  readonly modelId: string;
  readonly spawnSent: boolean;
  readonly resultReleased: boolean;
  readonly resultSent: boolean;
  readonly requestCount: number;
  readonly requests: readonly StubRequestClassification[];
}

export class LaneA4eBackendHarness {
  ...
  /** Agent Server 直连地址（app 与真值断言共用；本 lane 无代理） */
  get serverUrl(): string {
    return `http://127.0.0.1:${this.serverPort}`;
  }

  /** app 应当使用的地址（与 serverUrl 相同；launchApp 的 OPENCOLORFUL_SERVER_URL） */
  get appUrl(): string {
    return `http://127.0.0.1:${this.serverPort}`;
  }
  ...
  /** 配置 stub 脚本（spawn 参数 + 复位状态）；用例在拿到真实 providerId/modelId 后调用 */
  async configureScript(config: StubScriptConfigInput): Promise<void> {
    const response = await fetch(`http://127.0.0.1:${this.stubPort}/__a4e__/config`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(config),
      signal: AbortSignal.timeout(5_000),
    });
    if (!response.ok) throw new Error(`configureScript → HTTP ${response.status}`);
  }

  /** 放行被门挡住的 report_subagent_result 响应 */
  async releaseChild(): Promise<void> {
    const response = await fetch(`http://127.0.0.1:${this.stubPort}/__a4e__/release`, {
      method: "POST",
      signal: AbortSignal.timeout(5_000),
    });
    if (!response.ok) throw new Error(`releaseChild → HTTP ${response.status}`);
  }

  /** stub 请求分类日志（诊断/报告证据） */
  async stubState(): Promise<StubState> {
    const response = await fetch(`http://127.0.0.1:${this.stubPort}/__a4e__/state`, { signal: AbortSignal.timeout(5_000) });
    if (!response.ok) throw new Error(`stubState → HTTP ${response.status}`);
  ...
  async dispose(retain: boolean): Promise<{ cleaned: boolean; artifactsDir: string | null }> {
    ...
    if (retain) {
      const artifactsDir = path.join(ARTIFACTS_DIR, `oc-e2e-lane-a4e-retain-${this.label}-${Date.now()}`);
