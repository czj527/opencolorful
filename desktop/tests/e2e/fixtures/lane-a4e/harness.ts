/**
 * A4e lane · Playwright fixture 组合（与共享 fixtures/harness.ts / lane-a4b 同语义，绑定 lane 后端）。
 *
 * 生命周期：用例开始 → 拉起隔离 lane 后端（临时 OPENCOLORFUL_HOME + 脚本化 spawn stub + 真实 Agent Server）；
 * 用例结束 → 通过：删除临时目录并自检清理干净；失败：保留现场并把证据写入 desktop/test-artifacts/。
 */
import { test as base, expect } from "@playwright/test";

import { LaneA4eBackendHarness } from "./backend.js";

export interface LaneHarnessFixtures {
  /** 已就绪的隔离 lane 后端（serverUrl/appUrl/stubUrl/homeDir/userDataDir 可用） */
  lane: LaneA4eBackendHarness;
}

export const test = base.extend<LaneHarnessFixtures>({
  lane: async ({ }, use, testInfo) => {
    const lane = new LaneA4eBackendHarness(sanitizeLabel(testInfo.title));
    try {
      await lane.start();
      await use(lane);
    } finally {
