/**
 * B4/B5 lane · 后端 fixture（隔离生命周期，lane 本地实现）。
 *
 * 结构与 lane-b3/fixtures/backend.ts 同构（runRoot/home/user-data + ready 行解析 +
 * dispose retain/清理自检），差异：引导 lane-b45/server-bootstrap.ts（stub 支持
 * text / todo_tool 两种模式，无代理），app 直连 serverUrl。
 * 令牌经共享 fixtures/server-token.ts（#71）fail-closed 注入写请求。
 */
...
export interface StubOptions {
  readonly mode: "text" | "todo_tool";
  /** mode=text：回复文本（压缩场景用超长文本抬高 token 估算门槛） */
  readonly text?: string;
  /** mode=todo_tool：todo_write 的 arguments JSON */
  readonly todosJson?: string;
}

export class LaneB45BackendHarness {
  ...
  async setStub(options: StubOptions): Promise<void> {
    const response = await fetch(`${this.stubBase}/__b45__/config`, {
      method: "POST",
  ...
  /** stub 请求计数（诊断用） */
  async stubState(): Promise<{ mode: string; requestCount: number; todoCalls: number }> {
    const response = await fetch(`${this.stubBase}/__b45__/state`, { signal: AbortSignal.timeout(5_000) });
    if (!response.ok) throw new Error(`stubState → HTTP ${response.status}`);
    return await response.json() as { mode: string; requestCount: number; todoCalls: number };
  }
  ...
  async dispose(retain: boolean): Promise<{ cleaned: boolean; artifactsDir: string | null }> {
    ...
    if (retain) {
      const artifactsDir = path.join(ARTIFACTS_DIR, `oc-e2e-lane-b45-retain-${this.label}-${Date.now()}`);
