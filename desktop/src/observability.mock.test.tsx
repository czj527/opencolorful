...
/* ---------------------------------------------------------------------------
 * A5 lane（OBS-05：Desktop UI→日志关联）——日志页侧：
 * focus 诊断引用（错误行「在日志中查看」跳转带入）驱动活动 tab 的 traceId 预填，
 * queryActivity 真实携带 traceId 过滤参数（服务端 routes/observability.ts 已支持）；
 * 本地引用（origin=local）提示 shell.log 兜底，不混用 activity/audit 通道语义。
 * ------------------------------------------------------------------------- */

it("OBS-05: 诊断引用预填——活动 tab 按 traceId 预填并真实下发过滤参数，可清空还原", async () => {
  const tracker = trackConsoleErrors();
  const user = userEvent.setup();
  const calls: ActivityFilter[] = [];
  const source = overrideSource(new MockDataSource(), {
    queryActivity: (filter: ActivityFilter) => {
      calls.push(filter);
      const trace = filter.traceId ?? "";
      return Promise.resolve({
        rows: activityLogs.filter((row) => trace === "" || row.traceId === trace),
        nextCursor: null,
      });
    },
  });
  const focus = { traceId: "tr-9f2c71ab", origin: "server" as const, at: "2026-09-02T10:00:00+08:00" };
  let view = render(<LogsPage source={source} focus={focus} />);
  try {
    // 只剩该 trace 的 2 行（128/127），其他 trace 的行不再出现
    await screen.findByText("session.prompt.accepted");
    expect(activityTableRows()).toHaveLength(2);
    expect(screen.getByText("memory.recall.completed")).toBeTruthy();
    expect(screen.queryByText("tool.failed")).toBeNull();

    // 过滤参数真实下发 + 预填提示行（引用仅含 id 与时间戳，无敏感值）
    expect(calls[0]?.traceId).toBe("tr-9f2c71ab");
    expect(screen.getByText(/已按诊断引用预填 traceId 过滤/).textContent).toContain("tr-9f2c71");
    expect(screen.queryByText(/sk-|api key/i)).toBeNull();

    // 清空预填 → 重查全量
    await user.clear(screen.getByPlaceholderText("traceId"));
    await waitFor(() => {
      expect(activityTableRows()).toHaveLength(activityLogs.length);
    });
    expect(calls[calls.length - 1]?.traceId).toBeUndefined();
  } finally {
    view.unmount();
    tracker.restore();
  }

  // 本地引用：提示 shell.log 兜底（诊断通道语义，不与服务端 activity 混用）
  view = render(
    <LogsPage
      source={source}
      focus={{ traceId: "9a5b7c3d-0000", origin: "local", at: "2026-09-02T10:01:00+08:00" }}
    />,
  );
  try {
    await screen.findByText(/已按诊断引用预填 traceId 过滤/);
    expect(screen.getByText(/shell\.log/)).toBeTruthy();
  } finally {
    view.unmount();
  }
  tracker.expectNoErrors();
