...
本项目采用轻量流程，避免复杂 Skill 仪式和不可控 Token 消耗。默认协作分工如下：

- 主智能体只负责需求澄清、方案与计划、文档更新、任务拆分、子智能体协调、代码审查和最终验收；
- 子智能体负责具体实现和针对性测试。任务互不共享状态且无顺序依赖时，优先并行派发；会修改同一文件、共享迁移/协议或存在前后依赖时，必须串行；
- 子智能体不得扩大任务范围、修改无关文件或自行宣称 Phase 通过；返回时必须说明修改文件、验证命令、结果和未解决问题；
- 主智能体收到实现结果后必须独立检查 diff、运行验证命令并处理审查发现，不能把子智能体报告作为验收证据。

1. 确认当前分支和工作区，阅读任务范围、完成定义和验证命令，先指出会阻塞实现的计划错误。
2. 关键边界先写最小失败测试并确认 RED：Provider/Auth、Session 恢复、事件序号、Replay、Abort 竞态、WS 权限、A2UI/TokUI 安全。
3. 普通路由胶水、CLI 文案和简单映射不强制完整 TDD，但必须有相关验证并通过 TypeScript 严格检查。
4. 每次只实现当前 Phase 任务，不提前加入未排期的 Profile、记忆或多 Agent 能力。
5. 先跑任务指定测试，再单独运行 `npm run check`；Playwright 必须在 `web/` 目录执行 `npx playwright test`。
6. 每个任务独立提交，提交信息使用计划中给出的标题。
7. 更新当前 `plans/phase-xx.md` 的状态、真实提交、验证证据和已知偏差。
8. Phase 全部完成且验收通过后才更新主文档、创建完成标签并请求合并到 `main`。

除非用户明确要求，不为普通任务引入额外 worktree、复杂流程文档或重复计划；并行子智能体仅在任务独立且不产生共享状态冲突时使用。
