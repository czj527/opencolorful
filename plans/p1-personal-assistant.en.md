...
- 2026-08-27 T2 (lane B, subagent + main review) merged: native directory picker. Lane log: `plans/p1-t2-directory-picker.md`. (PR #18)
- 2026-08-27 T4 (lane D, subagent + main integration) merged: ID-card assistant card + thread grouping (进行中/最近/已归档); status line wired to real runtime state only. Lane log: `plans/p1-t4-idcard-sidebar.md`. (PR #19)
- 2026-08-27 T6 (lane F, subagent + main review) merged: centralized `errors.ts` mapping, offline/no-model send pre-checks, Chinese recovery copy + next-action affordances. Lane log: `plans/p1-t6-error-copy.md`. (PR #20)
- 2026-08-27 T5 (lane E, subagent + main integration) merged: `AgentProfilePage` (identity/persona/pinned/memory-settings), pinned POST/DELETE endpoints (plan-assumption correction: only GET existed — thin exposure of existing store), data-source write methods. Main-agent fixes: `source` wiring into profile page; mock pinned made mutable. CI episode (phase6 timeline flake, 4/4 on branch) resolved per g0 ledger round 8 → Playwright failure diagnostics (trace+artifact) added to CI. Lane log: `plans/p1-t5-profile-memory.md`. (PR #21)
- 2026-08-27 T1 (lane A, main agent): four-step onboarding wizard — create assistant (name + base-color template) → provider (presets + API key, saved at step 2) → working directory (T2 native picker + manual fallback) → plain-language permission explainer; assistant created only at the final step; completion re-derives first-run state and lands in a fresh conversation. Data source: `listAgentTemplates` + `createAgent` (ipc + mock). Lane log: `plans/p1-t1-onboarding.md`.
