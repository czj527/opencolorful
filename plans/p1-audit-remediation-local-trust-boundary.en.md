...
- **R3 (cross-lane integration, fixed on this branch):** the agent-settings fail-closed regression test merged from PR #65 (written against the pre-boundary baseline) used raw `createServerApp` with the fake `http://local` URL host; after the main merge it failed 4/4 with 403 (host `local` rejected by design, token absent). Migrated that spec to the central `createTrustedServerApp` harness and normalized its 5 `http://local` URLs to `127.0.0.1` — same adaptation applied to the 26 pre-existing files; verified 4/4 locally.
