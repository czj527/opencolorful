...
| 八 | 待作者执行（切片 1.5 已于 2026-08-28 全部合并：PR #26 T11 / #27 T7 / #28 T9 / #29 T8） | I 组回归（I7 按会话中心 IA 改写）+ 切片 1.5 验证（流畅度/零死控件/IA 自洽/环境坑消除） | 代码就绪，验收项见切片 1.5 规格 §四；lane log `plans/p1-t7/t8/t9/t11-*.md` | — |
...
### I 组：P1 切片 1（onboarding / 身份证卡 / 档案页 / 错误文案）— 待执行

> 来源：P1 切片 1 的 T0-T6（PR #16-#21）。执行需要真实 API Key（I3/I5/I14）。
> 干净首启用独立 `OPENCOLORFUL_HOME`；I12 用杀 4310 制造断线（属韧性操作，执行完须恢复）。

| ID | 步骤 | 预期 | 结果 | 证据 |
|---|---|---|---|---|
| I1 | 干净 OPENCOLORFUL_HOME 启动 Electron | 自动进入四步引导（无助理或无凭据即首启） | 待执行 | — |
| I2 | 第 1 步：空名点下一步被拦；输入名字 + 选底色模板 | 校验生效；模板卡选中态正确 | 待执行 | — |
| I3 | 第 2 步：选预设 + 粘贴 API Key → 下一步；再用错 Key 试一次 | 合法保存成功；错 Key 给中文错误不静默 | 待执行 | — |
| I4 | 第 3 步：点"浏览…" | 弹系统原生目录对话框；取消回退手动输入（T2） | 待执行 | — |
| I5 | 第 4 步：完成 → 进入对话页发首条消息 | 助理已创建并选中；真实流式回复 | 待执行 | — |
| I6 | 完成引导后重启应用 | 不再自动进引导（首启状态派生自真实数据，无持久化标记） | 待执行 | — |
| I7 | 侧栏身份证卡 | 名称/描述/真实状态（空闲/运行中/离线）正确；点击进档案页；切换助理可用 | 待执行 | — |
| I8 | 会话列表分组 | 流式中的会话进「进行中」，其余进「最近」，归档区不变 | 待执行 | — |
| I9 | 档案页：改名 + 改描述保存 | 侧栏/会话头同步；重启后保持 | 待执行 | — |
| I10 | 档案页/记忆页：置顶记忆添加、删除 | UI 更新 + curl 对照 `GET .../memory/pinned` 一致 | 待执行 | — |
| I11 | 档案页：记忆设置开关/时间 | PUT 生效（curl 对照 `GET .../memory/settings`） | 待执行 | — |
| I12 | 杀 4310 后发送消息 | "连接已断开"中文提示，无英文原文；恢复后自动重连 | 待执行 | — |
| I13 | 无已配置凭据时发送 | "还没有可用模型" + 去设置入口，不静默失败 | 待执行 | — |
| I14 | 记忆回环：会话中确认一个事实（如"我叫 X"），换新会话问"我叫什么" | 真实回想命中（非 mock） | 待执行 | — |
...
| 17 | **高** | 设置页死控件 25 个（设置页 24 + 侧栏 1）：9 个后端端点已存在但未接线、3 个本地显示偏好误当后端功能、13 个概念错位或功能不存在 | 作者真实初测 2026-08-27 | **已修复（2026-08-28 T8，PR #29）**：设置页四类目全接线——外观（主题+减少动效 localStorage）/模型与 Provider（新增全局默认模型选择器，PUT /api/settings/preferences 接线）/对话显示（思考+工具 toggle，ChatView 真正消费过滤）/关于（只读）；删除 8 个死类目。lane log `plans/p1-t8-settings-minimal.md`；待波次八作者真实点击复核 |
| 18 | **高** | SSE 流式渲染卡顿：每 token = 1 SSE → 1 IPC → projector O(n) 全量重建 → App 顶层 setState → 整树重渲染，无节流/memo/虚拟化 | 作者真实初测 2026-08-27 | **已修复（2026-08-28 T7，PR #27）**：事件合批节流 + projector O(1) 索引 + 组件 memo + 聊天状态下沉。lane log `plans/p1-t7-sse-perf.md`；流畅度主观验收待波次八 |
| 19 | 中 | IA 混乱：左上角助理卡片语义不明（侧栏切换助理只过滤列表，新建会话却要再选一次），"当前助理"是半个概念 | 作者真实初测 2026-08-27 | **已修复（2026-08-28 T9，PR #28，作者拍板 D1）**：去全局助理概念，会话中心 IA——会话行 badge、会话头 chip 进档案页、空态身份证卡+助理 chips、新建默认最近助理、新建助理入口补齐、档案页人设编辑。lane log `plans/p1-t9-session-centric-ia.md`；待波次八复核（I7 已按新 IA 改写） |
| 20 | 中 | 后端未连接时桌面端静默回退 mock，无醒目提示（以为在测真链路实际在点假数据）；`supervisor start` 不自动拉起 agent server（期望态 stopped，须手动 POST /api/supervisor/start） | 2026-08-27 排查作者"后端起不来"时发现 | **已修复（2026-08-28，PR #28/#26）**：T9 mock 醒目横幅（不可关闭，`.app` 条件三行 grid）；T11 supervisor start 期望态拉起 agent server（含 web E2E 竞态修复）。lane log `plans/p1-t9/t11-*.md`；待波次八复核 |
