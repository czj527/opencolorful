...
export const memoryAgentTools: readonly MemoryAgentTool[] = [
  ...
  make("propose_merge", "提议合并重复事实", (ctx, a) => { for (const factId of (a.payload.factIds as unknown[] ?? [])) requireOwnFact(ctx, String(factId)); const snapshots = (a.payload.factIds as unknown[] ?? []).map((factId) => { const f = ctx.factStore.getById(Number(factId)); return { id: String(factId), ...(f !== undefined ? { fact: f.fact, status: f.status, revision: f.updatedAt } : {}) }; }); return proposal(ctx, "merge", { payload: a.payload, evidenceRefs: a.evidenceRefs, reason: a.reason, confidence: a.confidence, targetType: "fact", previousState: { facts: snapshots } }); }),
