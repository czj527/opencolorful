...
/**
 * 表重建单步迁移（migrateTo13 / migrateTo14）的测试故障注入参数，生产路径恒不传。
 * afterRebuild 在该步骤全部重建 SQL 执行之后、版本号推进之前调用：抛错即令整步
 * 事务回滚，供故障注入回归测试验证"版本号不变、临时表消失、重跑迁移成功"
 * （tests/unit/storage/migrations-recovery.test.ts 场景 D）。
 */
export interface MigrationStepTestFault {
  afterRebuild?: () => void;
}
...
export function applyMigrations(database: Database.Database, observer?: MigrationObserver): void {
  // 引导段（v1）事务化（P0-2 审计同模式修复）：建表与版本号初始化原子生效。
  // 全部语句幂等（IF NOT EXISTS / 空表判重），中断后重跑无副作用。
  ...
  if (current < 2) {
    // P0-2 审计同模式修复：多语句步骤事务化 + ALTER 判重（中断恢复重跑不再因
    // duplicate column 失败），列变更与版本号推进同事务原子生效。
    database.transaction(() => {
      const sessionColumns = database.prepare("PRAGMA table_info(sessions)").all() as Array<{ name: string }>;
      if (!sessionColumns.some((column) => column.name === "tool_mode")) {
        database.exec("ALTER TABLE sessions ADD COLUMN tool_mode TEXT DEFAULT 'off'");
      }
      if (!sessionColumns.some((column) => column.name === "workspace_cwd")) {
        database.exec("ALTER TABLE sessions ADD COLUMN workspace_cwd TEXT");
      }
      if (!sessionColumns.some((column) => column.name === "workspace_confirmed")) {
        database.exec(
  ...
  if (current < 4) {
    database.transaction(() => {
      const sessionColumns = database.prepare("PRAGMA table_info(sessions)").all() as Array<{ name: string }>;
      if (!sessionColumns.some((column) => column.name === "agent_id")) {
  ...
  if (current < 7) {
    database.transaction(() => {
      ...
      // ALTER 判重（同 v9/v12 模式）：中断恢复重跑不因 duplicate column 失败。
      const journalColumns = database.prepare("PRAGMA table_info(memory_journal)").all() as Array<{ name: string }>;
      if (!journalColumns.some((column) => column.name === "priority")) {
        database.exec("ALTER TABLE memory_journal ADD COLUMN priority INTEGER NOT NULL DEFAULT 0");
      }
      database.exec(
  ...
  // P0-2 审计同模式修复：FTS 回填（INSERT ... SELECT）与版本号推进同事务，
  // 中断即整体回滚，重跑不会向 FTS 索引重复回填。
  if (current < 8) {
    database.transaction(() => {
      database.exec(`
  ...
  if (current < 13) {
    ...
    // 表重建拆为导出的单步函数 migrateTo13（事务化 + 临时表自愈 + 故障注入点，
    // P0-2 审计修复），供 applyMigrations 与恢复回归测试共同驱动。
    migrateTo13(database);
  }

  // v14：波次 A8 统一模型用量（语义见 migrateTo14 上方注释）。
  if (current < 14) {
  ...
  if (current < 15) {
    ...
    // 重跑本段时不会因 duplicate column 失败。P0-2 审计同模式修复：判重 ALTER、
    // session_todos 建表与版本号推进包进同一事务，中断即整体回滚（遗留的
    // session_todos 半成品由 CREATE TABLE IF NOT EXISTS 幂等吸收）。
    database.transaction(() => {
      const sessionColumns = database.prepare("PRAGMA table_info(sessions)").all() as Array<{ name: string }>;
...
// ─────────────────────────────────────────────────────────────────
// v13 / v14 表重建单步函数（P0-2 审计修复，
// plans/p1-audit-remediation-migration-recovery.en.md）。
//
// 修复前缺陷：重建序列（建临时表 → 拷贝 → DROP 旧表 → RENAME → 重建索引）以
// 多语句 autocommit 执行，版本号最后单独推进；进程中断后遗留临时表
// （memory_journal_v13 / usage_records_v14），下次启动重试在 CREATE TABLE 处报
// "table already exists"，数据库无法打开。
//
// 修复后契约：
// 1. 原子性：重建全部语句 + 版本号推进包在单一事务内，任意一步抛错整体回滚
//    （better-sqlite3 的 transaction 在异常时自动 ROLLBACK 并重抛）；
// 2. 自愈（对旧无事务实现中断遗留状态的恢复）：
//    a) 旧表缺失而临时表在（旧代码中断于 DROP 与 RENAME 之间）：临时表持有全量
//       拷贝数据，先 RENAME 回正名保住数据，再统一走标准重建；
//    b) 旧表与临时表都在（旧代码中断于拷贝阶段）：DROP 残留临时表，以旧表为准
//       重拷（即"CREATE TABLE <tmp> 前先 DROP TABLE IF EXISTS <tmp>"的判重恢复）。
// 3. 测试注入：testFault.afterRebuild 在重建完成后、版本推进前回调，抛错验证回滚。
// ─────────────────────────────────────────────────────────────────

/**
 * v13 单步迁移：memory_journal 表重建（actor CHECK 放开 'background_review'）。
 * SQLite CHECK 约束不可 ALTER，走标准表重建：建临时表 → 拷贝 → 换名 → 重建索引。
 * 单独导出供故障注入回归测试直接驱动，生产路径由 applyMigrations 调用。
 */
export function migrateTo13(database: Database.Database, testFault?: MigrationStepTestFault): void {
  database.transaction(() => {
    if (!tableExists(database, "memory_journal") && tableExists(database, "memory_journal_v13")) {
      // 旧代码中断于 DROP 旧表与 RENAME 之间：临时表持有全量数据，先归位再重建
      database.exec("ALTER TABLE memory_journal_v13 RENAME TO memory_journal;");
    }
    // 自愈审计前遗留状态：CREATE 前先清掉上次中断残留的临时表
    database.exec("DROP TABLE IF EXISTS memory_journal_v13;");
    database.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_journal_agent_priority ON memory_journal(agent_id, priority, status, created_at);
    `);
    // 测试注入点：重建 SQL 全部执行后、版本号推进前（抛错 → 整步回滚）
    testFault?.afterRebuild?.();
...
// 未知，不做猜测回填。
/**
 * v14 单步迁移：usage_records 表重建。单独导出供故障注入回归测试直接驱动，
 * 生产路径由 applyMigrations 调用（恢复契约同 migrateTo13）。
 */
export function migrateTo14(database: Database.Database, testFault?: MigrationStepTestFault): void {
  database.transaction(() => {
    if (!tableExists(database, "usage_records") && tableExists(database, "usage_records_v14")) {
      // 旧代码中断于 DROP 旧表与 RENAME 之间：临时表持有全量数据，先归位再重建
      database.exec("ALTER TABLE usage_records_v14 RENAME TO usage_records;");
    }
    // 自愈审计前遗留状态：CREATE 前先清掉上次中断残留的临时表
    database.exec("DROP TABLE IF EXISTS usage_records_v14;");
    database.exec(`
