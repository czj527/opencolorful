...
// ═══════════════════════════════════════════════════════════════
// P1 波次B B2 修复：共享 Runtime Bootstrap。
//
// 本模块承载 messages 路由的 Runtime 装配全量逻辑（工具面、记忆注入、
// Skill/Subagent 上下文、Agent profile/插件签名重建跟踪），由 app 组合根
// 构造单例后同时提供给 messages 路由与会话分支路由（regenerate/switch）。
// 分支路由懒创建 Runtime 时必须复用同一装配，杜绝"重启后首个动作是
// regenerate → Runtime 静默缺失插件/Skill 工具面"的降级。
// 代码自 messages.ts 原样抽取（verbatim move）：行为对既有 messages 路径
// 逐字节一致；两个跟踪 Map（runtimeSystemPrompt / pluginSignatures）的
// 全部读写点都在 ensureRuntime 闭包内，随本模块整体迁移。
// ═══════════════════════════════════════════════════════════════
...
export class EnsureRuntimeError extends Error {
...
export interface RuntimeBootstrap {
  /** 懒创建/按需重建 Session Runtime（messages 与分支路由共用）。 */
  readonly ensureRuntime: (sessionId: string) => Promise<void>;
  /**
   * 跟踪 Map 失效入口（语义与 ensureRuntime 重建前的 delete 对完全一致）：
   * 清除该会话的 Agent profile/插件签名跟踪，下一次 ensureRuntime 必然重建。
   */
  readonly forgetRuntimeTracking: (sessionId: string) => void;
  /** 只读窥视：该会话最近一次装配时使用的 systemPrompt（无跟踪时 undefined）。 */
  readonly peekTrackedSystemPrompt: (sessionId: string) => string | undefined;
  /** 只读窥视：该会话最近一次装配时的插件状态签名（无跟踪时 undefined）。 */
  readonly peekTrackedPluginSignature: (sessionId: string) => string | undefined;
}
