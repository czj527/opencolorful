...
function bindAuditOperations(inputs: readonly AuditRecordInput[]): AuditRecordInput[] {
  const correlationId = crypto.randomUUID();
  return inputs.map((input) => {
    const operationId = crypto.randomUUID();
    return {
      ...input,
      trace: { traceId: correlationId, spanId: operationId, correlationId, operationId },
    };
  });
}
...
export function registerAgentRoutes(
  ...
  app.post("/api/agents", async (context) => {
    try {
      ...
      const operationInputs = bindAuditOperations(auditInputs);
      if (auditInputs.length > 0) {
        if (audit === undefined) {
          return context.json(createApiError("PROVIDER_UNAVAILABLE", "安全审计不可用，高风险创建被拒绝"), 503);
        }
        const startedInputs: AuditRecordInput[] = operationInputs.map((input) => ({
          ...input,
        ...
        try {
          ...
          const failedInputs: AuditRecordInput[] = operationInputs.map((input) => ({
            ...input,
            eventName: input.eventName === "audit.agent.workspace_changed"
        ...
        try {
          audit.appendStrictMany(operationInputs);
        } catch {
