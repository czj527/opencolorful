...
5. **分层即遗忘**：不重要记忆随传送带折叠自然失去细节与可达性——遗忘的是"被想起的概率"，不是档案。不做评分式遗忘（强度/权重是否引入，由 Phase 10.5 评测台用数据裁决）。
...
| **10.5（原创+评价）** | **记忆 agent** 接管深潜（事实提取/冲突裁决/旧识失效标记，参考 hermes curator 后台 agent 模式）；**评测台**（persona 剧本植入事实 + 确定性断言 + 下钻路径验证）；强度/权重信号是否引入由评测数据裁决；记忆强度图 UI |
