...
| Product P1 | 个人助理基础体验 | 进行中（切片 1 + 切片 1.5 代码已合并，待作者波次八验收） | `docs/superpowers/specs/2026-08-26-p1-personal-assistant-slice.md`、`docs/superpowers/specs/2026-08-27-p1-slice-1.5-usability.md`、`plans/desktop-parity.md`、`plans/p1-t1~t9-*.md` |
...
1. **P1 切片 1.75 记忆激活（实施中）**：规格 `docs/superpowers/specs/2026-08-28-p1-slice-1.75-memory-activation.md`（2026-08-28 作者拍板），计划 `plans/p1-slice-1.75-memory-activation.en.md`。记忆骨架已有但行为层未激活，四个任务：T12 使用规则契约 / T13 工具 WHEN 引导 / T14 后台复盘 / T15 闭环实证。落地后作者真实日用观察记忆触发率，作为切片 2 记忆方向立项依据。
2. **P1 波次八验收（待作者执行）**：切片 1.5 已于 2026-08-28 全部合并（PR #26-#29）。作者按 `plans/desktop-e2e-test-plan.md` 执行 I 组回归（I7 按会话中心 IA 改写）+ 切片 1.5 验证项（流畅度/零死控件/IA 自洽/环境坑消除）。
