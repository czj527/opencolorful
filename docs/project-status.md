...
- **G1 仓库收敛与 Desktop 优先进行中**：desktop 为唯一产品前端，web 冻结为运维/测试面；治理规则重定位为单人 + AI agent 护栏。见 `plans/g1-repo-convergence.md`。
...
| Governance G1 | 仓库收敛与 Desktop 优先 | 进行中 | `plans/g1-repo-convergence.md` |
...
1. 完成 G1 文档收敛（单 PR，见 `plans/g1-repo-convergence.md` 任务清单）。
2. 建立 P1 实施计划（`plans/p1-personal-assistant.md`），调度切片 1 与 D 波次。
