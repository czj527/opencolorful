...
| 能力 | 难度 | 说明 |
|---|---|---|
| 记忆巩固（梦境 dreaming） | 中 | LLM 调用 + 评分逻辑 + cron 调度，工程可实现 |
...
**建议阶段**：Phase 10 先做案头/笺/今日记/往事/识见/梦境（巩固+自编辑+时序+全文），`手艺`技能自创留 Phase 11+，性格演变最后且谨慎（可能不做或做强约束）。

## 六、agent 成长/自我进化：主流怎么实现，难度大不大？

主流实现路径（由易到难）：
1. **记忆巩固**（openclaw dreaming）：后台 cron 跑三阶段，评分提升——中难度
2. **自编辑记忆**（Letta）：给 agent memory_replace/insert/rethink 工具，prompt 引导自主管理——中难度
3. **技能自创**（hermes curator + learning_mutations）：复杂任务后 agent 抽象出可复用技能，使用中改进，变异进化——高难度（评估闭环难）
...
- **现在做**（低成本、高价值）：
  ...
  - agent 关键行为埋点：工具调用、记忆读写、决策、subagent 委派、梦境巩固
  - 理由：agent 行为可观测是基础设施；后期补埋点很难（散落各处）
- **后续做**（过早优化）：
  - 完整 OpenTelemetry 分布式链路、性能 profiling、采样策略
  - 理由：现在 agent 自我层还在建，链路未稳定，过早 OTel 是负担

**建议**：从 Phase 9-10 起继续穿插搭"结构化日志框架 + 关键行为埋点"，不单列 Phase。日志框架成本低，但能避免后期补埋点的痛苦。
...
### 可简化
- **主/子 Agent 角色分工**：单人 + AI 协作开发时，"主 Agent/子 Agent"话术偏重。实际用 Agent 工具委派即可，无需严格角色流程。建议简化为"开发者 + AI 协作"，保留"AI 报告不作为验收证据，必须独立复核 diff + 重跑质量门"这条铁律
...
3. **Phase 10** 记忆系统（案头/笺/今日记/往事/识见/手艺/梦境）——核心
4. **结构化日志框架**：穿插在 8-10 实现中，不单列 Phase
